...
public class Bibliotheque {
	...
	/**
	 * L’ajout d'un nouveau document à la collection de documents. Au cas où il n’y
	 * a plus de place pour le document (si on dépasse la capacité de la
	 * bibliothèque), on doit lever une exception.
	 * 
	 * @param document
	 * @throws Exception
	 */
	...
	/**
	 * Seuls les livres peuvent être empruntés. Un ajustement du nombre de copies
	 * disponibles doit être effectué selon les prêts et les retours. Le prêt d’un
	 * livre donné revient à vérifier s’il a une copie disponible. Le retour d’un
	 * livre revient à rendre disponible une copie de ce livre. Au cas où l'on
	 * essaye d'emprunter un document qui n'est pas un livre, une exception doit
	 * être levée(signalée).
	 * 
	 * @param position
	 * @throws Exception
	...
	public void emprunter(int position) throws Exception {
		...
		Livre l;
		int nbDispo;
		if ((listeDocuments.get(position) instanceof Livre) && (position > 0)
				&& (position < this.listeDocuments.size())) {
	 ...
	 * disponibles doit être effectué selon les prêts et les retours. Le prêt d’un
	 * livre donné revient à vérifier s’il a une copie disponible. Le retour d’un
	 * livre revient à rendre disponible une copie de ce livre.
	 * 
	 * @param position
	 * @throws Exception
	 */
	...
	/**
	 * Le tri des documents par catégorie puis par code pour comparer deux objets :
	 * a.compareTo(b) == 0 si a.equals(b) a.compareTo(b) < 0 pour a strictement
	 * inférieur à b a.compareTo(b) > 0 pour a strictement supérieur à b
	 */
	 ...
	 * @throws IOException
	 */
	public void sauvegardeBinaire(ArrayList<Document> listeDocuments1, String path)
